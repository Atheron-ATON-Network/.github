...
**Outside pull requests are not being accepted while Atheroin is in development.** They will be closed without review. The reasoning is on the organization profile page, and it comes down to unstable interfaces and pending audits rather than any lack of interest.
...
**Two, progress is updated in the same pull request as the work.** `.atheroin/progress.json` lists build-plan items and their state. Do not batch these up to update later. The public completion percentage is computed from these files, so a stale one is a public misstatement.
